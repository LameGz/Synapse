...
# ─── Compute effective edges = depends_on + auto_linked ───────────────
effective_edges() {
  local deps="$1" auto_links="$2" merged="" item
  IFS=',' read -ra DEP_ITEMS <<< "$deps"
  for item in "${DEP_ITEMS[@]}"; do
    item=$(echo "$item" | xargs)
    [ -z "$item" ] && continue
    if ! echo ",$merged," | grep -qF ",$item,"; then
      merged="${merged:+$merged,}$item"
    fi
  done
  IFS=',' read -ra AUTO_ITEMS <<< "$auto_links"
...
# ─── Build tag index ───────────────────────────────────────────────────
declare -A TAG_MAP  # tag -> newline-separated "id | rel | deps | blocks | auto_linked | effective_edges | summary | keywords | aliases | tokens"
for rel in "${!NODE_INFO[@]}"; do
  ...
  effective=$(effective_edges "$deps" "$auto_linked")
  ...
  for tag in "${TAGS[@]}"; do
    ...
    TAG_MAP["$tag_lower"]="${TAG_MAP[$tag_lower]:-}${id}|${rel}|${deps}|${blocks}|${auto_linked}|${effective}|${summary}|${keywords}|${aliases}|${tokens}
"
  done
  ...
  if [ -n "$aliases" ]; then
    ...
    for al in "${ALS[@]}"; do
      ...
      if [ -z "${TAG_MAP[$al]:-}" ] || ! echo "${TAG_MAP[$al]}" | grep -qF "${id}|"; then
        TAG_MAP["$al"]="${TAG_MAP[$al]:-}${id}|${rel}|${deps}|${blocks}|${auto_linked}|${effective}|${summary}|${keywords}|${aliases}|${tokens}
"
      fi
    done
  fi
...
set +eu  # output block: allow unset variables and don't exit on pipeline/subshell errors
{
  ...
  if [ ${#TAG_MAP[@]} -eq 0 ]; then
    ...
    while IFS= read -r tag; do
      ...
      echo "### \`$tag\`"
      echo
      printf '%s' "${TAG_MAP[$tag]}" | sort -t'|' -k1,1 -u | while IFS='|' read -r nid nrel ndeps nblocks nauto_linked neffective nsummary nkeywords naliases ntokens; do
        ...
        [ -n "$neffective" ] && echo "  effective_edges: $(echo "$neffective" | sed 's/,/, /g')"
  ...
  if [ ${#ALL_NODES[@]} -eq 0 ]; then
    ...
    while IFS= read -r entry; do
      ...
      [ -n "$ndeps" ] && echo "  depends_on: $(echo "$ndeps" | sed 's/,/, /g')"
      [ -n "$nauto_linked" ] && echo "  auto_linked: $(echo "$nauto_linked" | sed 's/,/, /g')"
      neffective=$(effective_edges "$ndeps" "$nauto_linked")
      [ -n "$neffective" ] && echo "  effective_edges: $(echo "$neffective" | sed 's/,/, /g')"
      [ -n "$nblocks" ] && echo "  blocks: $(echo "$nblocks" | sed 's/,/, /g')"
  ...
  while IFS= read -r nrel; do
    ...
    neffective=$(effective_edges "${ndeps:-}" "${nauto_linked:-}")
    [ -n "${neffective:-}" ] && [ "$neffective" != " " ] && echo "  effective_edges: $(echo "$neffective" | sed 's/,/, /g')"
...

{
  ...
  for entry in "${ALL_NODES[@]}"; do
    ...
    effective_json=$(to_json_array "$(effective_edges "$ndeps" "$nauto_linked")")
    ...
    echo "      \"effective_edges\": $effective_json,"
