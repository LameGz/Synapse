...
parse_node() {
  ...
  local id type status updated tags depends_on summary aliases
  ...
  echo "${id:-unknown}|${type:-unknown}|${status:-unknown}|${updated:-}|${tags}|${depends_on}|${summary}|${keywords}|${aliases}|${tokens}|${rel}"
}
...
while IFS= read -r -d '' file; do
  ...
  if [ -n "$result" ]; then
    IFS='|' read -r id type status updated tags deps summary keywords aliases tokens rel <<< "$result"
    ALL_NODES+=("$result")
    NODE_DEPS["$rel"]="$deps"
    ...
    local ch_entries ch_date ch_summary ch_month
...
for rel in "${!NODE_INFO[@]}"; do
  ...
  IFS='|' read -r id type status updated tags deps summary keywords aliases tokens rel2 <<< "$result"
  ...
  for tag in "${TAGS[@]}"; do
    ...
    TAG_MAP["$tag_lower"]="${TAG_MAP[$tag_lower]:-}${id}|${rel}|${deps}|${blocks}|${summary}|${keywords}|${aliases}|${tokens}
"
  done
...
# ─── Write MEMORY_MAP.md ───────────────────────────────────────────────
set +u  # output block references many variables via echo; allow unset variables to expand to empty strings instead of failing
{
  ...
  if [ ${#TAG_MAP[@]} -eq 0 ]; then
    ...
    while IFS= read -r tag; do
      ...
      echo "### \`$tag\`"
      echo
      printf '%s' "${TAG_MAP[$tag]}" | sort -t'|' -k1,1 -u | while IFS='|' read -r nid nrel ndeps nblocks nsummary nkeywords naliases ntokens; do
  ...
  if [ ${#ALL_NODES[@]} -eq 0 ]; then
    ...
    while IFS= read -r entry; do
      [ -z "$entry" ] && continue
      IFS='|' read -r nid ntype nstatus nupdated ntags ndeps nsummary nkeywords naliases ntokens nrel <<< "$entry"
      case "$ntype" in
        module) pfx="[mod]" ;;
        feature) pfx="[feat]" ;;
        *) pfx="[$ntype]" ;;
  ...
  while IFS= read -r nrel; do
    ...
    IFS='|' read -r nid ntype nstatus nupdated ntags ndeps nsummary nkeywords naliases ntokens rel2 <<< "$result"
    [ -z "$nid" ] && continue
    # Count open issues
    issue_cnt=0
  ...
  while IFS= read -r nrel; do
    ...
    [ -z "$nrel_clean" ] && continue
    result="${NODE_INFO[$nrel_clean]}"
    IFS='|' read -r _ _ nstatus _ _ _ _ _ _ _ _ <<< "$result"
    [ -z "$nstatus" ] && continue
